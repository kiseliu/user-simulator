...
class ResponseDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, degree_size, dropout_rate, gru, proj, emb, vocab):
        ...
        self.proj_copy1 = nn.Linear(hidden_size, hidden_size)
        self.proj_copy2 = nn.Linear(hidden_size, hidden_size)
        self.proj_copy3 = nn.Linear(embed_size, hidden_size)
        self.dropout_rate = dropout_rate
    ...
    def forward(self, z_enc_out, u_enc_out, u_input_np, m_t_input, degree_input, last_hidden, z_input_np, **kwargs):

        # # adding goal for user utterance decoding
        goal_np = kwargs.get('goal', None)
        if goal_np is None:
            ...
            sparse_z_input = Variable(self.get_sparse_selective_input(z_input_np), requires_grad=False) # [B,T,V+T]
            ...
            goal = cuda_(Variable(torch.from_numpy(goal_np).long()))
            sparse_g_input = Variable(self.get_sparse_selective_input(goal), requires_grad=False)
...
class TSD(nn.Module):
    ...
    def forward_turn(self, u_input, u_len, turn_states, mode, degree_input, u_input_np, m_input_np=None,
        ...
        if prev_z_input is not None:
            pv_z_enc_out, _, pv_z_emb = self.u_encoder(prev_z_input, prev_z_len, enc_rand=True)
        if g_input is not None:
            g_enc_out, g_enc_hidden, g_emb = self.u_encoder(g_input, g_input_len, enc_rand=True)
            last_hidden_g = g_enc_hidden[:-1]
        u_enc_out, u_enc_hidden, u_emb = self.u_encoder(u_input, u_len, enc_rand=True)
            ...
            if cfg.dataset == 'usr_act':
                ...
                for t in range(m_length):
                    ...
                    proba, last_hidden, dec_out = self.m_decoder(pz_dec_outs, u_enc_out, u_input_np, m_tm1,
                                                                 degree_input, last_hidden, z_input_np, **kwargs)
                    if teacher_forcing:
            ...
            if cfg.dataset == 'usr_act' and mode == 'test':
                ...
                if not self.beam_search:
                    m_output_index = self.greedy_decode(pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden,
                                                        degree_input, bspan_index, **kwargs)
    ...
    def greedy_decode(self, pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden, degree_input, bspan_index, **kwargs):
        decoded = []
        ...
        for t in range(self.max_ts):
            proba, last_hidden, _ = self.m_decoder(pz_dec_outs, u_enc_out, u_input_np, m_tm1,
                                                   degree_input, last_hidden, bspan_index_np, **kwargs)
